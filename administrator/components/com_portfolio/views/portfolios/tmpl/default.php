...
$linkpost       = 'index.php?option='._JOOM_OPTION.'&view=portfolios';
...
if ($saveOrder)
{
    $saveOrderingUrl = 'index.php?option=com_portfolio&task=portfoliosort.saveOrderAjax&tmpl=component';
    JHtml::_('sortablelist.sortable', 'PortfoliosList', 'adminForm', strtolower($listDirn), $saveOrderingUrl, false, true);
}
?>
...
<div id="j-main-container" class="span10">
    <form id="adminForm" name="adminForm" method="post" action="<?php echo $linkpost ?>">
        ...
        <?php else : ?>
            <table class="table table-striped" id="PortfoliosList">
                <thead>
                <tr>
                ...
                if(!empty($this->items))
                    ...
                    {
                        $link_edit = 'index.php?option='._JOOM_OPTION.'&task=portfolio.edit&id='.$item->id;
                        $checkbox = JHtml::_('grid.id',$i,$item->id);
                        $rows     = 'row'.$i%2;
                        $title    = '<a href="'.$link_edit.'">'.$item->title.'</a>';
                        $status   = JHtml::_('jgrid.published',$item->published,$i,'portfolio.');
                        $id       = $item->id;
                        ?>
                        <tr class="row<?php echo $i % 2; ?>" item-id="<?php echo $item->id ?>" >
                            ...
                            <td  class="has-context">
                                <div class="pull-left break-word">
                                    ...
                                    <div class="small">
                                        <?php echo JText::_('Category') . ': ' . $this->escape($item->category_title); ?>
                                    </div>
                                </div>
                            </td>
