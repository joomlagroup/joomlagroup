...
*/
    ...
    protected function getInput()
    {
        ...
        $html = array();
        $link = 'index.php?option=com_jk8_products&amp;view=categories&amp;layout=modal'.
            '&amp;tmpl=component&amp;function=jSelectCategory_'.$this->id;

        $db = JFactory::getDbo();
        $query = $db->getQuery(true);
        $query->select('title');
        $query->from('#__products_categories');
        ...
        if (empty($title)) {
            $title = JText::_('COM_JK8_PRODUCT_FIELD_SELECT_CATEGORY');
        }
        ...
        $html[] = '    <a class="modal btn" title="'.JText::_('COM_JK8_PRODUCT_SELECT_CATEGORY_TITLE').'" href="'.$link.
            '" rel="{handler: \'iframe\', size: {x:800, y:450}}"><span class="icon-file" aria-hidden="true"></span>'.
            JText::_('COM_JK8_PRODUCT_BUTTON_SELECT_CATEGORY').'</a>';
